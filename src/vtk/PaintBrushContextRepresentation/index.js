...
function generateContour({ stencil, location, slicingIndex, indexToWorld }) {
  ...
  const getOrCreateVertexIndex = (x, y) => {
    ...
    // at x/y, the true continuous index value of the vertex is x-0.5/y-0.5
  ...
  for (let gy = 0; gy < gridydim; gy++) {
    for (let gx = 0; gx < gridxdim; gx++) {
      // evaluate point to see if it needs inclusion by looking at all
      // 4 pixels that share this point as a corner.
      // if all empty or all filled, the current point is not used
      // int the contour
      const count =
