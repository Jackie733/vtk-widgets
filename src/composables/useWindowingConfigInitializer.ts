...
import { WLAutoRanges, WL_AUTO_DEFAULT, WL_HIST_BINS } from '../constants';
import { getWindowLevels, useDICOMStore } from '../store/dicom';
import useWindowingStore from '../store/view-configs/windowing';
import { useWindowingConfig } from './useWindowingConfig';
...
export function useWindowingConfigInitializer(
  ...
  watchImmediate(imageData, (image) => {
    ...
    if (imageIdVal == null || config != null || !image) return;
  ...
  watch(autoRange, (percentile) => {
    ...
    const imageIdVal = unref(imageID);
    const viewIdVal = unref(viewID);
    if (imageIdVal == null || windowConfig.value == null || !image) return;
  ...
  useResetViewsEvents().onClick(() => {
    const imageIdVal = unref(imageID);
    const viewIdVal = unref(viewID);
    if (imageIdVal == null || windowConfig.value == null) return;
