...
import { createVTKImageThumbnailer } from '../core/thumbnailers/vtk-image';
...
const images = computed(() => {
  ...
  return nonDICOMImages.value.map((id) => {
    ...
    const layerable = id !== selectedImageID && primarySelection.value != null;
    return {
      id,
...
function setPrimarySelection(sel: DataSelection) {
...
<template>
  <div>
    ...
    <v-container v-show="images.length" class="p-0">
      <v-row no-gutters justify="space-between" align="center" class="mb-1">
        ...
        <v-col cols="6" align-self="center" class="flex justify-end">
          <v-btn
            icon
            variant="text"
          ...
          >
            ...
            <v-icon flip="vertical">mdi-lock-open-variant</v-icon>
            <v-tooltip
              :disabled="!sameSpaceImages"
              location="left"
              activator="parent"
              >Sync Images</v-tooltip
            >
          </v-btn>
          <v-btn
            icon
        ...
        >
          <div class="flex flex-row justify-between">
            <div class="allow-trunc-text-flex-child">
              <div class="font-bold text-nowrap text-truncate">
                {{ image.name }}
              </div>
              <div class="">Dims: ({{ image.dimensions.join(', ') }})</div>
              <div class="">Spacing: ({{ image.spacing.join(', ') }})</div>
            </div>
                ...
                <v-list>
                  ...
                  >
                    ...
                    <v-tooltip
                      ...
                      >Must load a background image before converting</v-tooltip
                    >
                  </v-list-item>
